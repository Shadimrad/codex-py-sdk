...
class ThreadRef:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ThreadRef":
        return cls(
            id=str(payload.get("id", "")), preview=str(payload.get("preview", ""))
        )
...
class TurnRef:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TurnRef":
        return cls(
            id=str(payload.get("id", "")), status=str(payload.get("status", "unknown"))
        )
...
class ThreadListResult:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ThreadListResult":
        return cls(
            data=[
                ThreadRef.from_dict(item or {}) for item in (payload.get("data") or [])
            ],
            next_cursor=(
                None
...
class ModelListResult:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ModelListResult":
        return cls(
            data=[
                ModelRef.from_dict(item or {}) for item in (payload.get("data") or [])
            ]
        )
...
class AgentMessageDeltaEvent:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "AgentMessageDeltaEvent":
        return cls(
            item_id=str(payload.get("itemId", "")), delta=str(payload.get("delta", ""))
        )
